...
! distan
!***************************************************************************************************
subroutine anagread(STAGFI,stid,sx,sy,slon,slat,shgt, KKA)
!-------------------------------------------------------------------------------
...
!===============================================================================
 ...
 do while (nlev /= 0)
  ...
  !    VARS   29
  ! 01 idro    0  99  idrometro
  ! 02 idro2   0  99  idrometro 2
  ! 03 pluvio  0  99  pluviometro
  ! 04 pluvio2 0  99  pluviometro 2
  ! 05 temp    0  99  termometro
  ! 06 temp2   0  99  termometro 2
  ! 07 vvc     0  99  velocita vento
  ! 08 dvc     0  99  direzione vento
  ! 09 vvq     0  99  velocita vento q
  ! 10 dvq     0  99  direzione vento q
  ! 11 vvs     0  99  velocita vento sonica
  ! 12 dvs     0  99  direzione vento sonica
  ! 13 vvp     0  99  velocita vento prevalente
  ! 14 dvp     0  99  direzione vento prevalente
  ! 15 press   0  99  pressione
  ! 16 ur      0  99  umidita relativa
  ! 17 rg      0  99  radiazione globale
  ! 18 rn      0  99  radiazione netta
  ! 19 so2     0  99  so2
  ! 20 no2     0  99  no2
  ! 21 no      0  99  no
  ! 22 nox     0  99  nox
  ...
  ! 25 pm10    0  99  pm10
  ! 26 pm102   0  99  pm102
  ! 27 elev    0  99  elevazione ASL [m]
  ! 28 rete    0  99  codice rete
  ! 29 wlu     0  99  peso UHI
  !------------------------------------------------------------
 ...
 !-----------------------------------------------------------------------
 ! coordinate conversion GB-> lon,lat
 do k=1,KKA
  tGB%E = sx(k)
  tGB%N = sy(k)
  tGB%S = WEST
...
!-------------------------------------------------------------------------------
 return
end subroutine anagread
!***************************************************************************************************
subroutine gridread(OROGRIFI, II, g4x, g4y, g4lon, g4lat, g4hgt, g4msk)
!-------------------------------------------------------------------------------
! GB
!-------------------------------------------------------------------------------
 use coords
 ...
 real(4), dimension (IM) :: g4x, g4y, g4lon, g4lat, g4hgt, g4msk
...
!-----------------------------------------------------------------------
 ...
 read (3,rec=2) g4msk
 close (3)
 write (6,*) 'gridread: orography read, now return, ok?'
! read (5,'(a1)') ans
!-------------------------------------------------------------------------------
 return
